                        ...
                        <div v-if="rowe.length<=1  && rowe[0].statutVisite===3">
                            <div v-if=" checkArray(roles,controleur)" class="card mb-20">
                                <a href="#"  @click="signin()" class="item item-text-wrap item-button-left  taille">
                                    <i class="i-Finger-Print icon"></i> <span class="icons">{{$t('sign_inspection')}}</span>
                                </a>
                            </div>
                        </div>
                    ...
                    <b-tabs pills >
                        ...
                        <b-tab title="" active>
                            ...
                            <b-overlay :show="loadanotherpage" rounded="sm" >
                                ...
                                <ListTable :rows="list" :columns="columns" :isCLoseMenu="true" @searchlistticket="searchlistticket"
                                           :totalElement="totalElement" :links="links"
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters(["getLink","GetVueKanban","GetVueGraph","GetVueHist","getVisiteWaiting","getSideBarToggleProperties","getStatusApprove"]),
        },

        methods: {

            ...mapActions(["FetchVueKanban","FetchVueGraph","visiteenattente","approve","bilancaisse","printp"]),
            searchlistticket(params){
                this.loadanotherpage = true
                this.visiteenattente(params)
            },
            rowClick(data, index) {
            ...
            printpv(){
                ...
                axios.get(constants.resource_url+'visites/imprimer/pv/'+this.rowe[0].id)
            ...
            clickRow(params){
                ...
                this.tests[4] = "G";*/
                    ...
                    /*axios.get(constants.resource_url+"filenames")

                    .then(holds => {
                        if(holds.data.success) {
                            console.log('filenames',holds.data.data);
                            this.filenames = holds.data.data;
            ...
            /*checkIds(obj, id) {
                ...
                var valObj = obj.filter(function(index,elem){
            ...
            checkIds(obj, id) {
                ...
                return obj.map(function(item) { return item.id; }).indexOf(id);
               // return obj.map(function(index,item) {if(item.id === id){return index;} });
            ...
            setupStream() {
                ...
                console.log('cocole', Vue.prototype.$keycloak.token)

                /*axios.get(constants.resource_urls+"/public/subscribe"
                )
                    .then(message => {
                ...
                es.addEventListener('new_visit', message => {
                    let data = JSON.parse(message.data);

                    if(data!=null) {
                        var arrays = [];
                        arrays.push(data);
                        this.list = (arrays).concat(this.list);

                        this.totalElement++
                    }

                    var audio = new Audio(require('../../../plugins/mixkit-tile-game-reveal-960.wav'));
                    audio.play();
                ...
                es.addEventListener('edit_visit', message => {
                  ...
                  //  this.stockData = data.stockData;
                    ...
                    console.log('ssemessages10',data);

                    let indexe =0;
                    indexe = this.checkIds(this.list,(data.id))
                    console.log('ssemessages10',indexe);

                    if(indexe > -1){
                        //this.list.$set(indexe, (message.body.data)[0]);
                        this.list.splice(indexe, 1, (data))
            ...
            handleEditVistEvent(message){
                console.log('ssemessage',message);
                console.log('ssemessage',message.id);
                let indexe =0;
                indexe = this.checkIds(this.list,(message.id))
                if(message.statutVisite===2){
                    indexe = this.checkIds(this.list,(message.id))
                }
                if(message.statutVisite===1){
                    indexe = this.checkIds(this.list,(message.id))
                }
                if(indexe > -1){
                ...
                const index = this.checkIds(this.list,(message.id))
                if(index > -1){
                    //this.list.$set(indexe, (message.body.data)[0]);
                    this.list.splice(index, 1, (message))
                    //this.list.splice(indexe, 1, updatedList)
                }

                console.log('sses', index!=-1)
...
=======
            /*this.$sse(constants.resource_urls+"/public/subscribe", { format: 'json' }).then(sse => {
>>>>>>> 52c23d0170410f53e78ee93f68208b2c1e49a0d2
                console.log('sse',sse);
                sse.subscribe('new_visit', (message) => {
                    console.log('ssemessage',message);
                    if(message!=null) {
                        var arrays = [];
                        arrays.push(message);
                ...
                sse.subscribe('edit_visit', (message) => {
                    console.log('ssemessage',message);
                    console.log('ssemessage',message.id);
                    let indexe =0;
                    indexe = this.checkIds(this.list,(message.id))
                ...
                sse.subscribe('edit_visit', (message) => {
                    ...
                    //console.log('sselist', (this.list[0].id))
                    console.log('sse', (message.body.data).id)

                    const indexe = this.checkIds(this.list,(message.body.data).id)
                    if(indexe > -1){
                        //this.list.$set(indexe, (message.body.data)[0]);
                        this.list.splice(indexe, 1, (message.body.data))
                        //this.list.splice(indexe, 1, updatedList)
                    }

                    console.log('sses', indexe!=-1)
           ...
           /* this.$sse(constants.resource_urls+"/public/subscribe", { format: 'json' })
                .then(sse => {
                    console.log('Connected!');
                    sse.subscribe('error', (e) => {
                    ...
                    sse.subscribe('new_visit', this.handleNewVistEvent);
                    // Handle 'edit_visit' messages
                    sse.subscribe('edit_visit', this.handleEditVistEvent);
                    // Unsubscribes from event-less messages after 1 hour
                    setTimeout(() => {
                        sse.unsubscribe('new_visit', this.handleNewVistEvent);
                        console.log('Stopped listening to new visit!');
                    }, 3600000);

                    // Unsubscribes from chat messages after 1 hour
                    setTimeout(() => {
                        sse.unsubscribe('edit_visit', this.handleEditVistEvent);
                        console.log('Stopped listening to edit visit messages!');
                    }, 3600000);
                })
                .catch((err) => {
        ...
        watch: {
            getLink(value){
                ...
                // this.$store.state.visits.link=""
                //window.open(constants.resource_urls+value, '_blank')
